...
export const updateMatchesInDb = async (
  req: Request,
  res: Response,
  next: Function
) => {
  try {
    ...
    await asyncForEach(matches, async (match: string) => {
      ...
      query = `UPDATE public."Matches" SET match${i} = '${JSON.stringify(
        data.info
      )}'::jsonb WHERE puuid = '${puuid}';`;
...
export const getUserRanked = async (
  req: Request,
  res: Response,
  next: Function
) => {
  try {
    ...
    if (user) {
      ...
      const summoner = await getSummonerEntriesByAccountIdAndRegion(
        data.id,
        regionCode
      );
...
export const getSummoner = async (
  ...
  try {
    ...
    let rank = await getSummonerEntriesByAccountIdAndRegion(
      summoner.data.id,
      region
    );
    ...
    await asyncForEach(matches, async (match: string) => {
      ...
      query = `UPDATE public."Matches" SET match${i} = '${JSON.stringify(
        data
      )}'::jsonb WHERE puuid = '${summoner.data.puuid}';`;
      await sequelize.query(query);
